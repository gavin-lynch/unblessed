/**
 * Dashboard UI (browser only). Loaded by `main.ts` after a DOM check so CLI
 * runners (`bun main.ts`, `node main.ts`) fail with a clear message instead of
 * xterm's `document is not defined`.
 *
...
// Register platform runtime before any widgets touch getRuntime().
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (!lastBg || lastBg[0] !== r || lastBg[1] !== g || lastBg[2] !== b) {
        acc += `\x1b[48;2;${r};${g};${b}m`;
        lastBg = [r, g, b];
      }

      if (y === labelY && x >= labelX && x < labelX + label.length) {
        if (lastFg === "default") {
          acc += "\x1b[38;2;250;250;250m";
          lastFg = [250, 250, 250];
        }
        acc += label[x - labelX];
      } else {
...
function setLineData(mockData: LineSeriesData[], line: Line) {
